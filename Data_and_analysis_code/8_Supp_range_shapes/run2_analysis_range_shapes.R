...
range_shape <- 'ellipse'
  # ellipse, chull, geosphere
  # if none of this selected, a rectangular is assumed

if (by_species){
  print('by species')
  data_sum <- read.csv('data_working/data_sum_species_copy.csv', header = TRUE, sep = "\t")  
}else{
  print('by genera')
  data_sum <- read.csv('data_working/data_sum_genera_copy.csv', header = TRUE, sep = "\t")
}

pdf('plots/fig_shape_rec_ell.pdf',height = 4.5, width = 4)
plot(data_sum[,'sp_area_square'],data_sum[,'sp_area_el'],pch = 16,xlab = 'Rectangular range area, Mkm2', ylab = 'Elliptic range area, Mkm2',xlim = c(0,250), ylim = c(0,250))
abline(coef = c(0,1),col = 'grey')
dev.off()

pdf('plots/fig_shape_ell_chull.pdf',height = 4.5, width = 4)
plot(data_sum[,'sp_area_el'],data_sum[,'sp_area_chull'],pch = 16,ylab = 'Convex hull range area, Mkm2', xlab = 'Elliptic range area, Mkm2',xlim = c(0,200), ylim = c(0,200))
abline(coef = c(0,1),col = 'grey')
dev.off()

pdf('plots/fig_shape_chull_geo.pdf',height = 4.5, width = 4)
plot(data_sum[,'sp_area_chull'],data_sum[,'sp_area_geosphere'],pch = 16,xlab = 'Convex hull range area, Mkm2', ylab = 'Geosphere range area, Mkm2',xlim = c(0,150), ylim = c(0,150))
abline(coef = c(0,1),col = 'grey')
dev.off()


if (range_shape=='geosphere'){
  print('range by true area')
  data_sum[,'sp_area_square'] <- data_sum[,'sp_area_geosphere']
  data_sum[,'sp_range_width'] <- sqrt(data_sum[,'sp_area_square'])
}

if (range_shape=='chull'){
  print('range by convex hull, even degrees to km')
  data_sum[,'sp_area_square'] <- data_sum[,'sp_area_chull']
  data_sum[,'sp_range_width'] <- sqrt(data_sum[,'sp_area_square'])
}

if (range_shape=='ellipse'){
  print('range by ellipse')
  data_sum[,'sp_area_square'] <- data_sum[,'sp_area_el']
  data_sum[,'sp_range_width'] <- sqrt(data_sum[,'sp_area_square'])
}
...
file1 <- paste('outputs/law_duration_results_',sp,'.csv',sep='')
file2 <- paste('outputs/law_range_results_',range_shape,'_',sp,'.csv',sep='')
